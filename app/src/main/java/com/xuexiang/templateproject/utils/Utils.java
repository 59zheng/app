...
//                不显示拍照按钮
    ...
    //    选择图片danzhang
    public static PictureSelectionModel getPictureSelectorOne(Fragment fragment) {
        return PictureSelector.create(fragment)
                .openGallery(PictureMimeType.ofAll())
                .theme(SettingSPUtils.getInstance().isUseCustomTheme() ? R.style.XUIPictureStyle_Custom : R.style.XUIPictureStyle)
                .maxSelectNum(1)
...
//                不显示拍照按钮
    ...
    //    拍照
    public static PictureSelectionModel getPictureSelectorpaizhao(Fragment fragment) {
        return PictureSelector.create(fragment)
